...
# ---------------- contained code to open in the browser ---------
...
# 2) Open a URL or Linux path in Windows (handles \\wsl.localhost\…)
.wsl_open <- function(u) {
  if (grepl("^https?://", u)) {
    system2("cmd.exe", c("/C","start","", u), wait = FALSE)
  } else {
...
# b) Plot test — if httpgd is installed, this appears in the same browser tab
plot(1:10, main = "httpgd browser plotting test")
#### ---------- end self-test ----------
...
# ---- just a fialed attempt - delete when you see fit



install.packages(c("Rcpp","jsonlite","later","promises","httpuv","bslib"))
install.packages(c("systemfonts","textshaping"))  # uses the font libs you installed
install.packages("httpgd")

options(repos = c(
  nx10      = "https://nx10.r-universe.dev",          # author’s universe (hosts unigd)
  community = "https://community.r-multiverse.org",   # community mirror (hosts httpgd)
  CRAN      = "https://cloud.r-project.org"
))
install.packages(c("unigd","httpgd"))
...
options(
  viewer = function(u) {
    if (grepl("^https?://", u)) {
      ...
      system2("cmd.exe", c("/C","start","", win), wait = FALSE)
    }
  },
  menu.graphics = FALSE
)
...
plot(1:10)                     # base
# ggplot2 etc. will stream to the same browser tab
...
# 1) start the httpgd graphics device + server
httpgd::hgd(port = 0)   # no 'open' argument in this version

# 2) open the viewer (uses options('viewer') function if set)
httpgd::hgd_view()      # or: httpgd::hgd_browse()
# If that doesn't open, use this explicit fallback:
system2("cmd.exe", c("/C","start","", httpgd::hgd_url()), wait = FALSE)
...
options(repos = c(CRAN = "https://cloud.r-project.org"), menu.graphics = FALSE)
options(repos = c(CRAN="https://cloud.r-project.org"), menu.graphics = FALSE,
        browser="/usr/bin/wslview", viewer=NULL)
