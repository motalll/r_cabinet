...
invisible(try(Sys.setlocale("LC_ALL", "C.UTF-8"), silent = TRUE))  # soften locale noise
...
## Utility: safely write data.frames with list-columns (flatten them)
safe_write_csv <- function(x, file, ...) {
  df <- as.data.frame(x)
  for (nm in names(df)) {
    if (is.list(df[[nm]])) {
      df[[nm]] <- vapply(df[[nm]], function(v) paste(v, collapse = ";"), character(1))
    }
  }
...
write.csv(res_df,  "DE_full_results.csv", row.names = FALSE)
write.csv(sig_res, "DE_significant_padj_lt_0.05.csv", row.names = FALSE)
...
# Hallmark gene sets (msigdbr: use 'collection', not deprecated 'category')
...
if (have_CP) {
  ...
  safe_write_csv(as.data.frame(gsea_res), "GSEA_hallmark_clusterProfiler_results.csv")
  if (have_EP) {
  ...
  fg <- fg[order(fg$padj), ]
  safe_write_csv(fg, "GSEA_hallmark_fgsea_results.csv")  # flattens list columns (e.g., leadingEdge)
  # quick dotplot
  topn <- head(as.data.frame(fg), 20); topn$pathway <- factor(topn$pathway, levels = rev(topn$pathway))
